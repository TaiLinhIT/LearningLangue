...
public class AdminLearningService(IDbContextFactory<LanguageLearningDbContext> factory) : IAdminLearningService
{
    public async Task<IReadOnlyList<User>> GetUsersAsync(CancellationToken cancellationToken = default)
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken);
        return await db.Users.AsNoTracking().OrderByDescending(x => x.CreatedAt).ToListAsync(cancellationToken);
    }

    public async Task<IReadOnlyList<Course>> GetCoursesAsync(CancellationToken cancellationToken = default)
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken);
        return await db.Courses.Include(x => x.Language).AsNoTracking().OrderBy(x => x.Title).ToListAsync(cancellationToken);
    }

    public async Task<IReadOnlyList<Unit>> GetUnitsAsync(CancellationToken cancellationToken = default)
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken);
        return await db.Units.Include(x => x.Course).AsNoTracking().OrderBy(x => x.CourseId).ThenBy(x => x.SortOrder).ToListAsync(cancellationToken);
    }

    public async Task<IReadOnlyList<Lesson>> GetLessonsAsync(CancellationToken cancellationToken = default)
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken);
        return await db.Lessons.Include(x => x.Unit).ThenInclude(x => x!.Course).AsNoTracking().OrderBy(x => x.UnitId).ThenBy(x => x.SortOrder).ToListAsync(cancellationToken);
    }

    public async Task<IReadOnlyList<Vocabulary>> GetVocabularyAsync(CancellationToken cancellationToken = default)
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken);
        return await db.Vocabulary.Include(x => x.Lesson).AsNoTracking().OrderBy(x => x.Word).ToListAsync(cancellationToken);
    }

    public async Task<IReadOnlyList<Question>> GetQuestionsAsync(CancellationToken cancellationToken = default)
    {
        await using var db = await factory.CreateDbContextAsync(cancellationToken);
        return await db.Questions.Include(x => x.Lesson).Include(x => x.Options).AsNoTracking().OrderBy(x => x.LessonId).ToListAsync(cancellationToken);
    }
